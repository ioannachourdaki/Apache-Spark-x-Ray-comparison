...
# Function that sorts a ray dataset
def sort_ds(ds):
    return ds.sort("trip_time", descending=False)
...
# Used to run the sort and calculate the memory it used at the same time
query_mem, processed_dataset = memory_usage((sort_ds,(ray_dataset,)), retval=True)
...
with open('sortRayResults.txt', 'a') as file:
    file.write(
        f"\nRun Sort\n"
